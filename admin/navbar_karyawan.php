<?php
/**
 * File: admin/navbar_karyawan.php
 * Deskripsi: Navigasi khusus Staf/Kasir
 */
...
<nav id="smartNavbar" class="navbar navbar-dark py-3 px-4 sticky-top">
    <div class="d-flex align-items-center w-100">
        <h4 class="mb-0 fw-bold me-5"><span class="text-accent">O2 FUTSAL</span> | STAF</h4>
        
        <div class="d-none d-lg-flex me-auto" style="position: relative; z-index: 60;">
            <a href="dasbor_karyawan.php" class="nav-link-custom <?php echo $halaman_aktif == 'dasbor_karyawan.php' ? 'active fw-bold' : ''; ?>">Jadwal Hari Ini</a>
            <a href="data_transaksi.php" class="nav-link-custom <?php echo $halaman_aktif == 'data_transaksi.php' ? 'active fw-bold' : ''; ?>">Transaksi Kasir <?php echo $badge_html; ?></a>
            <a href="status_lapangan.php" class="nav-link-custom <?php echo $halaman_aktif == 'status_lapangan.php' ? 'active fw-bold' : ''; ?>">Status Lapangan</a>
        </div>

        <div class="d-flex align-items-center gap-3" style="position: relative; z-index: 60;">
            <span class="text-secondary small">Halo, <span class="text-white fw-bold"><?php echo ucwords($_SESSION['peran'] ?? 'Staf'); ?></span></span>
            <a href="../logout.php" class="btn btn-sm btn-danger rounded-pill px-3 btn-hover-float" onclick="return confirm('Keluar dari Dasbor Staf?');">Keluar</a>
        </div>
    </div>
</nav>
